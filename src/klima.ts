...
export class Klima {
  ...
  constructor(private fetcher: FetchClient, private ea: EventAggregator) {
    ...
    this.livingroom_gauge.lowerBands = [{from: 20, to: 30, color: "#DC3912"}, {
      from: 30,
      to: 40,
      color: "#ffd74c"
    }, {from: 40, to: 60, color: "#109618"},
  ...
  async update() {
    const inside_temp = await this.fetcher.fetchValue(`${globals.iobroker}/get/${globals._livingroom_temp}`)
    const inside_humid = await this.fetcher.fetchValue(`${globals.iobroker}/get/${globals._livingroom_humidity}`)
    const outside_humid = await this.fetcher.fetchValue(`${globals.iobroker}/get/${globals._outside_humidity}`)
    const outside_temp = await this.fetcher.fetchValue(`${globals.iobroker}/get/${globals._outside_temp}`)
    const bathroom1_temp = await this.fetcher.fetchValue(`${globals.iobroker}/get/${globals._bathroom_temp}`)
    const bathroom1_humid = await this.fetcher.fetchValue(`${globals.iobroker}/get/${globals._bathroom_humidity}`)
    const bright = await this.fetcher.fetchValue(`${globals.iobroker}/get/${globals._brightness}`)
